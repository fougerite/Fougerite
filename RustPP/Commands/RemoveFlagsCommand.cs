    ...
    public class RemoveFlagsCommand : ChatCommand
    {
        public override void Execute(ref ConsoleSystem.Arg Arguments, ref string[] ChatArguments)
        {
            ...
            if (match.Count == 1)
            {
                ...
                RemoveFlags(match[0], Arguments.argUser);
                return;
            }
            ...
            Core.adminUnflagWaitList[Arguments.argUser.userID] = admins;
        ...
        {             
            ...
            foreach (string properName in flags)
            {
                ...
                {
                    Util.sayUser(myAdmin.networkPlayer, Core.Name, "You can't remove the RCON flag to anyone's permissions.");
                    Administrator.NotifyAdmins(string.Format("{0} attempted to remove the {1} flag to {2}'s permissions.", myAdmin.displayName, properName, administrator.DisplayName));
                ...
                } else
                {
                    ...
                    Administrator.NotifyAdmins(string.Format("{0} removed the {1} flag to {2}'s permissions.", myAdmin.displayName, properName, administrator.DisplayName));
                    if (properName == "RCON")
                    {                           
